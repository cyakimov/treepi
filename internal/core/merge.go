...
func (s *Service) Merge(ctx context.Context, task string) (*MergeResult, error) {
	...
	preCtx := s.baseHookContext(ctx, config.EventPreMerge, task, t.Type, branch, dir)
	preCtx.Op, preCtx.OldHead, preCtx.NewHead = opID, trunkOID, rebasedOID
	if spec, herr := s.fireHook(ctx, preCtx); herr != nil {
	...
	postCtx := s.baseHookContext(ctx, config.EventPostMerge, task, t.Type, branch, dir)
	...
	if _, herr := s.fireHook(ctx, postCtx); herr != nil {

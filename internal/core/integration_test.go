...
// TestIntegrationNewCutsFromLocalTrunk encodes the dogfooding finding: when local
// trunk is ahead of origin/<trunk> (an unpushed commit), `new` must branch from
// the LOCAL trunk, not the stale remote tip. Regression for resolveBase.
func TestIntegrationNewCutsFromLocalTrunk(t *testing.T) {
	dir := newRepo(t)
	ctx := context.Background()

	// A bare remote with main pushed, then main tracks origin/main.
	remote := t.TempDir()
	run(t, remote, "init", "-q", "--bare", "-b", "main")
	run(t, dir, "remote", "add", "origin", remote)
	run(t, dir, "push", "-q", "origin", "main")
	run(t, dir, "branch", "--set-upstream-to=origin/main", "main")

	// Advance LOCAL main past origin/main with an unpushed, trunk-only marker.
	if err := os.WriteFile(filepath.Join(dir, "LOCAL_ONLY.txt"), []byte("y\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	run(t, dir, "add", ".")
	run(t, dir, "commit", "-q", "-m", "local-only commit")

	svc := openSvc(t, ctx, dir)
	if _, err := svc.New(ctx, "feat", "x"); err != nil {
		t.Fatalf("New: %v", err)
	}
	// The worktree must contain the local-only marker, proving it was cut from
	// local main and not the stale origin/main.
	if _, err := os.Stat(filepath.Join(dir+".worktrees", "x", "LOCAL_ONLY.txt")); err != nil {
		t.Fatalf("worktree was not cut from local trunk (LOCAL_ONLY.txt missing): %v", err)
	}
}
...
func TestIntegrationNewListWhere(t *testing.T) {
	...
	if info.Branch != "feat/login" || info.Status != "ready" {
	...
	// A second task coexists.
	if _, err := svc.New(ctx, "fix", "bug"); err != nil {
		t.Fatal(err)
	}

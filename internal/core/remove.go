...
// worktree the caller is standing in, and refuses a dirty tree without --force.
// It always snapshots first, so even a forced discard is recoverable via undo;
// on a forced discard it warns which gitignored paths will not be recoverable.
func (s *Service) Remove(ctx context.Context, task string, force bool) (*RemoveResult, error) {
	...
	hc := s.baseHookContext(ctx, config.EventPreRemove, task, t.Type, branch, dir)
	hc.Op = opID

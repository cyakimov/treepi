...
// StateDir is where treepi keeps its manifest and journal.
...
// defaults to "{type}/{task}". An empty type collapses cleanly: "{type}/{task}"
// with no type yields just "{task}". The rendered name is validated by the
// caller via git check-ref-format.
func BranchName(template, typ, task string) string {
	...
	// An empty {type} can leave a leading or doubled slash (e.g. "/login"); tidy
	// it so `new <task>` with no type yields a clean branch name.

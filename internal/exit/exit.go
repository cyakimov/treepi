...
// The exit-code taxonomy. Scripts branch on these without parsing text. Codes
// 5-7 (claim/slot/lease) were retired with those features; the remaining codes
// keep their stable values so existing consumers of exit 8+ are unaffected.
const (
	OK           Code = 0
